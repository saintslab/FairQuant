...
#   uniform-4, uniform-8, FairPrune, HAWQ, FQ-QAT, FQ-BAQ
...
# --- FairPrune (gradient*weight importance, subtractive across sensitive groups) ---
run "${COMMON[@]}" --quant_mode fair_static_qat \
    --importance_metric gradient --reducer subtractive --importance_on_sensitive_groups \
    --quant_bits $QUANT_BITS --quant_levels $QUANT_LEVELS --ft_epochs "$FT_EPOCHS" \
    --run_name "${PREFIX}_fairprune"

# --- HAWQ (Hutchinson-trace Hessian importance, not fairness-aware) ---
run "${COMMON[@]}" --quant_mode fair_static_qat \
    --importance_metric hawq --reducer max \
    --quant_bits $QUANT_BITS --quant_levels $QUANT_LEVELS --ft_epochs "$FT_EPOCHS" \
...
for seed in "${SEEDS[@]}"; do
    run "${COMMON[@]}" --quant_mode baq_learnable \
        --importance_metric gradient --reducer max --importance_on_sensitive_groups \
        --quant_bits 2 4 8 16 --quant_levels 0.25 0.25 0.25 0.25 \
        --baq_bit_min 4 --baq_bit_max 16 --baq_lambda_b 1e-5 --fairness_loss_lambda 0.5 \
